...
node --env-file=.env.local scripts/fetch-flickr-photos.mjs [ids…]  # same, from Flickr (needs FLICKR_API_KEY)
...
Policy: **free licenses only** (CC0/CC BY/CC BY-SA/PD/US-Gov). Two sources share one disambiguated query map (`scripts/_idol-search.mjs`): **Wikimedia Commons** (`fetch-idol-photos.mjs`, no key) and **Flickr** (`fetch-flickr-photos.mjs`, needs `FLICKR_API_KEY`; license-gated to ids 4/5/7/8/9/10 — the no-NC/no-ND set). Flickr is a bigger but noisier pool, so the same eyeball-every-file rule applies; Flickr-sourced attribution rows carry `"source": "flickr"`. News sites and Instagram are copyrighted — not "free" even when not fancams — so they are out of bounds for the repo; gaps get hand-dropped officials instead. Per-photo attribution is recorded in `public/idols/attribution.json`. Idols without a photo render gradient-initial cards (`Thumb`); `image_focus` (0–1, default 0.3) keeps faces at consistent height via object-position and survives re-syncs.
