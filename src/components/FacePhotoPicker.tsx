...
export default function FacePhotoPicker({
  ...
  if (raw) {
    return (
      <div className="space-y-3 rounded-xl border border-[#c8ccd2] bg-white/70 p-3">
        ...
        <input aria-label="照片縮放" type="range" min={1} max={3} step={0.05} value={zoom} onChange={(event) => setZoom(Number(event.target.value))} className="w-full accent-[#b4302b]" />
        <div className="flex gap-2">
          <button type="button" disabled={busy} onClick={confirm} className="flex-1 rounded-lg bg-[#1c1e24] py-2 text-xs font-bold text-white disabled:opacity-50">{busy ? "裁切中…" : "使用這張照片"}</button>
          <button type="button" onClick={() => setRaw(null)} className="rounded-lg border border-[#c8ccd2] px-4 text-xs font-bold">取消</button>
        </div>
        {failed && <p role="alert" className="text-xs text-[#b4302b]">照片處理失敗，請換一張再試。</p>}
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {value && <button type="button" onClick={() => onChange(null)} className="text-xs text-[#9aa0aa] hover:text-[#b4302b]">移除</button>}
      {failed && <span role="alert" className="text-xs text-[#b4302b]">讀取失敗</span>}
    </div>
  );
}
